import {
  ...
  getArrayPow,
  getArraySqrt,
} from '/@/custom-server/core/utils/SciLab';
import type { MovementModel } from '../Models/Movement';
import type { SensorModel } from '../Models/Sensor';
...
export default new (class SciLabServices {
  async getAllCalc(movements: MovementModel[], newSession: SessionModel) {
    ...
    for (const movement of movements) {
      if (movement.sensors) {
        const calc = await this.calculationVariabilityCenter(
          movement.sensors,
          newSession,
          'eChart',
          movement,
  ...
  async calculationVariabilityCenter(
    sensors: SensorModel[],
    session: SessionModel,
    chartType = 'eChart',
    movement: MovementModel,
  ): Promise<CalculationVariabilityCenter | null> {
