...
class Connection {
  ...
  constructor(client, io) {
    ...
    this.client.on('measurements', (object) => this.measurements(object))
    // Ao desconectar
    this.client.on('disconnect', (e) => this.disconnect(e))
    // Ao ter error
  ...
  measurements(object) {
    ...
    //       const array = typeof data === 'string' ? JSON.parse(data) : data !== null ? data : []
    //       if (this.isSensor) {
  ...
  disconnect(e) {
    console.log(translate('socket.disconnect'), e)
