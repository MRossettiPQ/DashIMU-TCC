...
async function start() {
  try {
    ...
    await execSync("npm install -g yarn@latest electron-packager@latest copyfiles@latest", { stdio: "inherit" });
    ...
    await execSync("yarn --cwd ../client client:build", { stdio: "inherit" });
    switch (process.platform) {
      case "linux":
        //TODO move os arquivos da pasta de build do quasar para a pasta public de distribuição do express
        await execSync(`mv -v ../client/dist/spa/* ../${process.env.TYPE}/src/custom-server/public`, { stdio: "inherit" });
        break;
      case "win32":
        //TODO deleta a versão anterior do SPA presente na pasta de distribuição do server
        await execSync(`del /S /Q ${path.resolve(`../${process.env.TYPE}/src/custom-server/public`)}\\*`, { stdio: "inherit" });

        //TODO move os arquivos da pasta de build do quasar para a pasta public de distribuição do express
        await execSync(`copyfiles "${path.resolve("../client/dist/spa")}\\*" "${path.resolve(`../${process.env.TYPE}/src/custom-server/public/`)}"`, { stdio: "inherit" });
        break;
      default:
    ...
    if (process.env.TYPE === "electron") {
      await execSync("yarn --cwd ../electron build", { stdio: "inherit" });
    }
