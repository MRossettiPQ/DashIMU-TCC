...
module.exports = new (class SciLabService {
  getAllCalc(movements, newSession) {
    let result = [];

    for (const movement of movements) {
      const calc = this.calculationVariabilityCenter(movement.sensors, newSession, "eChart");
      result.push({
        movement,
        ...calc,
      });
    ...
    //       try {
    //         const calc = await this.calculationVariabilityCenter(m.sensors, newSession, "eChart");
    //       } catch (e) {
    //         console.log(e);
    //       }
    //     });
  ...
  calculationVariabilityCenter(sensors, session, chartType = "eChart") {
    ...
    if (sensors?.length) {
      ...
      if (sensor_1 !== null && sensor_2 !== null) {
        ...
        switch (chartType) {
          case "eChart":
            chartOption = this.getEChartOptions(session, max_atorn);
            break;
          default:
    ...
    //         switch (chartType) {
    //           case "eChart":
    //             chartOption = this.getEChartOptions({
    //               session,
    //               max_atorn,
    //             });
    //             break;
  ...
  getEChartOptions(session, max_atorn) {
