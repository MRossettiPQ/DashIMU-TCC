...
export default new (class FilterUtils {
  date(dateIso) {
    const format = dateIso.length > 1 && dateIso[1] !== undefined ? dateIso[1] : "DD/MM/YYYY";
  ...
  dateTime(dateIso) {
    const format = dateIso.length > 1 && dateIso[1] !== undefined ? dateIso[1] : "DD/MM/YYYY HH:mm";
  ...
  yearMonth(dateIso) {
    const format = dateIso.length > 1 && dateIso[1] !== undefined ? dateIso[1] : "MM/YYYY";
  ...
  time(dateIso) {
    const format = dateIso.length > 1 && dateIso[1] !== undefined ? dateIso[1] : "HH:mm";
  ...
  secondsToTime(seconds) {
    const concatHyphen = seconds.length > 1 && seconds[1] !== undefined ? seconds[1] : false;
  ...
  money(value) {
    const decimalPlaces = value.length > 1 && value[1] !== undefined ? value[1] : 2;
    ...
    if ((value + "").indexOf(".") !== -1) {
      ...
      const missingDecimalPlaces = decimalPlaces - value.substring(value.indexOf(",") + 1, value.length).length;

      if (missingDecimalPlaces > 0) {
        value = "".concat(value).concat(Array(missingDecimalPlaces).fill("0").join(""));
      }
