...
export default class ValidatorsUtils {
  ...
  maxLength(maxLength) {
    return function (val) {
      return !val || val.length <= maxLength || "Tamanho máximo de ".concat(maxLength, " excedido.");
    };
  }

  notBlankIf(condition) {
    return function (val) {
      return !condition || typeof val == "number" || !_.isEmpty(val) || "Preenchimento obrigatório.";
    };
  }

  notBlank(val) {
    return typeof val == "number" || !_.isEmpty(val) || "Preenchimento obrigatório.";
  }
  ...
  maxVal(maxValue) {
    return function (value) {
      ...
      if (!isValid) {
        return "O valor deve ser menor ou igual a ".concat(FilterUtils.money(maxValue), ".");
      }
    };
  }
  ...
  dateBorn(value, minAge = 18) {
    return moment(value, "YYYY/DD/MM").isBefore(moment().subtract(minAge, "years")) || `Deve ser maior de ${minAge} anos.`;
  }
