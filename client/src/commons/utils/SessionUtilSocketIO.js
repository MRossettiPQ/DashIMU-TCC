...
class BackEndSocketUtil {
  ...
  connect(socket_url = "localhost:8000") {
    ...
    this.socket.on("disconnect", () => {
      this.notify(closeMessage, "positive");
    });
    ...
    this.socket.on("sensor-disconnected", (json) => {
      console.log("sensor-disconnected");
      console.table(json);
      this.checkIfSensorIsConnected(json.data.sensor);
    });
  ...
  requestAvailableSensors() {
    ...
    this.send(this.socket, "get-sensor-list", "GET_UPDATE_CLIENT_LIST");
    this.loadingSensors = true;
  }

  send(socket, event, type, message = null) {
    if (socket && event) {
      socket.emit(event, JSON.stringify({ origin: "FRONT", type, message }));
    }
  }
