import { Database, LoadedCustomModels } from "../database";
import { ContextRequest, throwError } from "../utils/RequestUtil";
import { NextFunction, Response } from "express";

const { User, Patient }: LoadedCustomModels = Database.models;
export async function VerifyUserEmailDuplicate(req: ContextRequest, res: Response, next: NextFunction) {
...
export async function verifyExistsCPFinPatient(req: ContextRequest, res: Response, next: NextFunction) {
  if (req.body) {
    return await throwError({
      local: "SERVER:VALIDATOR-USER",
      message: "Nenhum dado valido foi enviado",
