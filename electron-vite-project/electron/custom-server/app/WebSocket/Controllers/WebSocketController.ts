...
import { ContextRequest, throwSuccess } from "../../../core/utils/RequestUtil";
...
export default new (class WebSocketController {
  async metadata(req: ContextRequest) {
    return await throwSuccess({
      content: {
        socket_url: `${req.socket.localAddress}:${environment.host.port}`,
...
function sendMessageAllClients(expressWs: ExpressWebSocket, type: string, message: string | SensorSocket[] | SensorSocket) {
...
function sendMessageToClient(client: SocketConnection, type: string, message: string | SensorSocket[] | SensorSocket) {
  client.send(
    JSON.stringify({
      origin: "SERVER",
      type,
