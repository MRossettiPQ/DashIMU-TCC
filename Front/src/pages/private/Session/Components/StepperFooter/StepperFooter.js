...
class StepperFooter extends Vue {
  @Prop()
  sessionConnection;
  ...
  addMeasurement() {
    if (this.sessionConnection.numberOfMeasurements > 0) {
      if (this.syncedSession.running_movement !== null) {
        this.syncedSession.addSensorsToMovement(
          this.sessionConnection.registeredSensorsList
        );
        Notify.create({
  ...
  get disableNextButton() {
    switch (this.navigation.actualStepValue) {
      ...
      case "select-sensor":
        return (
          this.sessionConnection?.numberOfValidConnection <
            this.syncedSession?.minSensor ||
          this.sessionConnection?.numberOfValidConnection >
            this.syncedSession?.minSensor ||
  ...
  get checkPositionBlank() {
    return this.sessionConnection.registeredSensorsList.some(
      (sr) => sr.position === ""
    );
  }
}
